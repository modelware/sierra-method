// adapted from: https://github.com/anareaty/obsidian-snippets/blob/main/Dataview-interactive-tables/dvit-info-en.md
...
class dvIT {
            ...
            new Setting(contentEl)
              ...
              .addToggle((toggle) => {
                ...
                toggle.onChange((toggleValue) => {
                  ...
                  let excludeOn = document.querySelector(
                    "." +
                      lineClass +
              ...
              .addToggle((toggle) => {
                ...
                toggle.onChange((toggleValue) => {
                  ...
                  let includeOn = document.querySelector(
                    "." +
                      lineClass +
  ...
  async changeView(dv, id) {
    let currentFile = await app.vault.getAbstractFileByPath(
      ...
      for (let key of keyWords) {
        if (key.length > 0) {
          pages = pages.filter((p) =>
            p.file.name.toLowerCase().includes(key.toLowerCase())
          );
        }
      }
    }
            ...
            filteredPages = filteredPages.map((p) => {
              ...
              for (let f of includeFilter) {
                if (f == "-" && (!val || val.length == 0)) {
                  ...
                  f.path &&
                  val &&
                  val.find((link) => link.path == f.path)
                ) {
                  return p;
            ...
            filteredPages = filteredPages.map((p) => {
              ...
              for (let f of excludeFilter) {
                if (f == "-" && (!val || val.length == 0)) {
                  ...
                  f.path &&
                  (!val || !val.find((link) => link.path == f.path))
                ) {
                  return p;
  ...
  async filterButton(p, pages, container, id, className) {
    ...
    if (multiSelect) {
      ...
      if (
        (current[propNameInclude] && current[propNameInclude].length != 0) ||
  ...
  async changeProp(p, pages, id) {
    ...
    if (propType == "text") {
      ...
      let valueNames = values.map((v) => {
        ...
        if (
          filter &&
          (filter == v ||
            (filter.path && "[[" + filter.path.replace(".md", "") + "]]" == v))
        ) {
          v = v + " ✔";
        }
        return v;
    ...
    if (propType == "multitext" && multiSelect) {
      ...
      let newFilter = await this.multiSuggestDouble(
        name,
        valueNames,
        multiValues,
        filter
  ...
  async newEntryButton(dv, args, container) {
    ...
    const createNote = async () => {
      let template = await app.vault
        .getFiles()
        .find((f) => f.path == noteTemplate);
      let data = "";
      if (template) {
  ...
  async createList(props, pages, filteredPages, paginationNum, id) {
    ...
    if (paginationNum) {
      filteredPages = await this.paginate(
        filteredPages,
        paginationNum,
        current,
        id
    ...
    for (let page of filteredPages) {
      ...
      let propItem = props.find(
        (p) => p.prop == "link" && p.type == "file prop"
      );
    ...
    let rows = filteredPages.map((p) =>
      tableProps.map((propItem) => {
            ...
            if (p.path) {
              ...
              propItem.replace.forEach((r) => {
                if (r[2] == "icon") {
                  ...
                  if (iconEl)
                    r[1] = iconEl.outerHTML.replace(
                      "svg-icon",
                      "svg-icon dv-tag dv-tag-" + r[0]
                    );
                }
        ...
        if (propType == "text" && propItem.textHeight) {
          if (!propVal) propVal = "";
          propVal =
            "<div class='longText' style='max-height: " +
            propItem.textHeight +
            "px'>" +
            propVal +
            "</div>";
        ...
        if (!propName.startsWith("file.")) {
          if (
            ...
            (propType == "text" || propType == "multitext")
          ) {
...
*/
  ...
  async renderView(settings, props, pages, dv) {
    ...
    filteredPages = await this.filterProps(
      props,
      dv.current(),
      filteredPages,
      id
    ...
    if (!view || view == "table") {
      ...
      await this.createTable(
        ...
        id,
        fullWidth,
        { cards: true, position: cardsPosition }
      );
    } else if (view == "list") {
